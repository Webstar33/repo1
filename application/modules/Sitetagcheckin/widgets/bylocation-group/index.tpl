...
<script type="text/javascript">
  ...
  var paginatePageLocations = function(page) {
		var  formElements = document.getElementById('group_filter_form');
    ...
    var url = en4.core.baseUrl + 'widget/index/mod/sitetagcheckin/name/bylocation-group';
    clearOverlays();
    gmarkers = [];
  ...
  <a id="eventlocation_location_map_anchor" class="pabsolute"></a>
		<?php if (count($this->paginator) > 0): ?>
			<ul class="seaocore_browse_list" id="seaocore_browse_list"><?php if (!empty($this->is_ajax)) : ?>	
				...
				<?php echo $this->translate(array('%s group found.', '%s groups found.', $this->totalresults),$this->locale()->toNumber($this->totalresults)) ?>
				</p></li>
				<?php foreach ($this->paginator as $item): ?>
				<?php if(!empty($item->location) || !empty($this->locationVariable)) :  ?>
					<li>
						<div class="seaocore_browse_list_photo">
							<?php echo $this->htmlLink($item->getHref(), $this->itemPhoto($item, 'thumb.normal'), array('title' => $item->getTitle(), 'target' => '_parent', 'class' => !empty($item->location)? "seao_common_add_tooltip_link marker_photo_".$item->group_id :'un_location_eventlocation seao_common_add_tooltip_link', 'rel'=>'group'.' '.$item->group_id)); ?>
						</div>

						<div class='seaocore_browse_list_info'>
							<div class='seaocore_browse_list_info_title'>
								<?php echo $this->htmlLink($item->getHref(), $item->getTitle(), array('title'
									=> $item->getTitle(), 'target' => '_parent', 'class' =>!empty($item->location)? "seao_common_add_tooltip_link marker_".$item->group_id :'un_location_eventlocation seao_common_add_tooltip_link', 'rel'=>'group'.' '.$item->group_id)); ?>
							</div>
							<div class="seaocore_browse_list_info_date">
							<?php echo $this->translate(array('%s member', '%s members', $item->membership()->getMemberCount()),$this->locale()->toNumber($item->membership()->getMemberCount())) ?>
							<?php echo $this->translate('led by');?> <?php echo $this->htmlLink($item->getOwner()->getHref(), $item->getOwner()->getTitle()) ?>
							</div>
              <?php if(!empty($item->location)): ?>
								...
								<?php if (!empty($item->distance) && isset($item->distance)): ?>
									<div class="seaocore_browse_list_info_stat">
										<?php $flage = Engine_Api::_()->seaocore()->geoUserSettings();
										if (!$flage): ?>
...
<script type="text/javascript" >
  ...
  window.addEvent('domready',function() {
    //smallLargeMap(1);
  //  var Clientwidth = $('global_content').getElement(".layout_sitetagcheckin_bylocation_group").clientWidth;
...
<?php if (!empty($location->location)) : ?>
	...
	var page_id = '<?php echo  $location->group_id;  ?>';
	var sponsored = 0;
	// create the marker

	<?php $page_id = $location->group_id; ?>

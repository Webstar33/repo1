 ...
 */
class Sitetagcheckin_CheckinController extends Core_Controller_Action_Standard {
  ...
  public function suggestAction() {
    ...
    //RETRVING CURRENT LOCATION BY USING LATITUDE & LONGITUDE
    if(empty($text) && !$tagged_location) {
    $url = sprintf("https://maps.googleapis.com/maps/api/geocode/json?latlng=%s,%s", $latitude, $longitude);
    $content = file_get_contents($url); // get json content
    $metadata = json_decode($content, true); //json decoder
        if(count($metadata['results']) > 0) {
         $text = $metadata['results'][1]['formatted_address'];
        }
    }

...
ActivityUpdateHandler = new Class({
  ...
  checkFeedUpdate : function(action_id, subject_guid){
    ...
    /***************SD Dev1 custom code for feed filters**************/
    ...
    var locationParams = "";
    try{
        cat = window.feedFilterCat;
        subCat = window.feedFilterSubCat;
    ...
    var req = new Request.HTML({
      url : en4.core.baseUrl + 'widget/index/name/sd-feed',
      data : {
        'format' : 'html',
        'minid' : min_id,
        'feedOnly' : true,
      ...
      (function() {
        if( this.options.showImmediately && $('feed-update').getChildren().length > 0 ) {
          ...
          this.getFeedUpdate(this.options.next_id);
              ...
              try{
                    if($('activity-feed') && $('activity-feed').getElement("li")){

...
//     exit;
        ...
        if ($params['isOnline'] == 1) {
            $select->setIntegrityCheck(false)->joinLeft($onlineTableName, ("$onlineTableName.user_id = $tableName.subject_id"));
            $select->where("$onlineTableName.user_id > ?", 0);
        }
        if ($params['isOnline'] == 2) {
            $select->where("$tableName.subject_id NOT IN (SELECT user_id from `engine4_user_online` where user_id > 0)");
        }
        $select->where("`$tableName`." . '`action_id` IN(' . join(',', $ids) . ')');
        ...
        if (!empty($params['objectType']) && $params['objectType'] !== 'all' && $params['objectType'] !== 'status') {

            $select->where("object_type = ?", $params['objectType']);
        }

        if (!empty($params['objectType']) && $params['objectType'] == 'status') {

            $select->where("type = ?", $params['objectType']);
        }
        ...
        if ($params['popular'] == 1) { // get most popular post
            $ids = Engine_Api::_()->adcustommodule()->getAllPapular();
            $idsss = join(',', $ids);
            $select->where('action_id IN(' . join(',', $ids) . ')');
            $select->order(new Zend_Db_Expr("FIELD( action_id, $idsss)"));
        }

        if (!empty($params['new_post']) && $params['new_post'] == 1) { // get new post
            $select->order("action_id DESC");
        }

        if (!empty($params['mostpopular']) && $params['mostpopular'] == 1) {
            $ids = Engine_Api::_()->adcustommodule()->recordFilterHistory();
            // $ids = Engine_Api::_()->adcustommodule()->getMostPoplularFeed();
            if (count($ids) > 0) {
                $idsss = join(',', $ids);
        ...
        if (!empty($params['locationParams']) && $params['mostpopular'] == 1) {
        ...
        if (!empty($params['locationParams']) && $params['popular'] == 1) { // top most in the location on the behalf of like and comment
            $locationParams = (array) @json_decode($params['locationParams']);
            if (!empty($locationParams['latitude']) && !empty($locationParams['longitude'])) {
                $radius = 10; //in miles
                $latitude = $locationParams['latitude'];
                ...
                $ids = Engine_Api::_()->adcustommodule()->getAllPapularByTheLocation($radius, $latitude, $longitude);
                $idsss = join(',', $ids);
        ...
        if (!empty($params['locationParams']) && !empty($params['new_post'])) {
            $locationParams = (array) @json_decode($params['locationParams']);
            if (!empty($locationParams['latitude']) && !empty($locationParams['longitude'])) {
                $radius = 10; //in miles
                $latitude = $locationParams['latitude'];

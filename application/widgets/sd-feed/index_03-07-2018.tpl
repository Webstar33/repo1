...
<script type="text/javascript" src="//code.jquery.com/jquery-1.11.0.min.js"></script>
  ...
  <script type="text/javascript" src="slick/slick.min.js"></script>
   ...
   <script type="text/javascript">
    ...
    /* jQuery('a.subscription_button_element').click(function(e){
          subscribeNotificationChannel(e.target);
    }); */
    jQuery(document).ready(function(){
		
		
		
		jQuery('#notifications_menu').click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation()
			var element = e.target;
			
			var channel_id = jQuery(e.target).closest('li').data('channel_id');
			console.log(channel_id);
			var url = '<?php echo $this->url(array('module' => 'adcustommodule', 'controller' => 'index', 'action' => 'get-channel-information','content_id' => $this->identity), 'default', true) ?>';
			...
			var request = new Request.HTML({
				...
				onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript) {
					loader.destroy();
					var Channel_obj = jQuery.parseJSON(responseHTML);
					
					jQuery("#locationParams").attr('value',Channel_obj.location_params);
					window.feedFilterCat = Channel_obj.category_id;
					window.feedFilterSubCat = Channel_obj.subcategory_id;
					window.popularFeedFilter = Channel_obj.popular;
					window.mostPopularFeedFilter = Channel_obj.most_popular;
					window.objectType = window.objectType;
					window.new_post = window.new_post;
					window.isOnline = window.isOnline;
					window.locationFeedFilter = Channel_obj.location_params;
					
					loadChannelData(Channel_obj.category_id, Channel_obj.subcategory_id, Channel_obj.popular, Channel_obj.most_popular, Channel_obj.location_params);
					
					formatted_address = (Channel_obj.location_params == '')?'Worldwide':jQuery.parseJSON(Channel_obj.location_params).formatted_address;

					jQuery('.sd_feed_worldwide > a').html(formatted_address);
										
					if(Channel_obj.most_popular != 0){
						jQuery('.sd_feed_mostpopular > a').html('Most Popular');
					}
					if(Channel_obj.popular != 0){
						jQuery('.sd_feed_mostpopular > a').html('Classic');
					}					
					if(Channel_obj.popular == 0 && Channel_obj.most_popular == 0){
						jQuery('.sd_feed_mostpopular > a').html('New');
					}
					
					jQuery('.sd-category-'+Channel_obj.category_id+'-item').find('a').trigger('click');
		...
		jQuery(".sd_feed_categories").css("width",'85%');
...
function loadChannelData(category_id, subcategory_id, popular, mostpopular, locationParams){
    ...
    var url = '<?php echo $this->url(array('module' => 'core', 'controller' => 'widget', 'action' => 'index', 'content_id' =>1051), 'default', true) ?>';         
    console.log(url);
    var request = new Request.HTML({
          url : url,
          data : {
...
<div class='sd_feed_header'>
    <div class='sd_feed_headersection sd_feed_mostpopular'>
        <a href='javascript:void(0);' class="sd_active_pouplar" onclick='loadMostPopularFeed(this);'><?php echo $this->translate("Most Popular"); ?></a>
        <ul>
            <li class='active'>
                <a href='javascript:void(0);' onclick='activateFeedOptions(this);loadMostPopularFeed(this);'><?php echo $this->translate("Most Popular"); ?></a>
            </li>
            <li>
                <a href='javascript:void(0);' onclick='activateFeedOptions(this);loadAllFeed(this);'><?php echo $this->translate("New"); ?></a>
            </li>
            <li>
                <a href='javascript:void(0);' onclick='activateFeedOptions(this);loadPopularFeed(this);'><?php echo $this->translate("Classic"); ?></a>
            </li>
        </ul>
    </div>
    <div class='sd_feed_headersection sd_feed_worldwide just-to-check'>
    ...
    <div class='sd_feed_headersection sd_feed_options'>
		...
		<div id="sd_filter_online_offline">
			...
			<ul id="online_user_list">
				...
				<li class="">
					<a href="javascript:void(0);" onclick="loadAllOnlineFeed(this, 1);"><?php echo $this->translate("Online"); ?><span class="ad_green"></span></a>
				</li>
				<li>
...
<script type='text/javascript'>
function subscribeNotificationChannel(element){
	/* if(!$('activity-feed')){
        return;
    }
    if(window.mostLocationFeedRequest){
    ...
    var filters = $("sd_feed_filters");
    var subject_guid = '<?php echo $this->subjectGuid ?>';
    var url = '<?php echo $this->url(array('module' => 'adcustommodule', 'controller' => 'index', 'action' => 'subscribe-channel','content_id' => $this->identity), 'default', true) ?>';         

    var typelabel = jQuery('.sd_feed_mostpopular > a').html();
    var locationlabel = jQuery('.sd_feed_worldwide > a').html();
    ...
    var request = new Request.HTML({
          ...
          onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript) {
                loader.destroy();
				alert('You are subscribed to this channel')
...
function loadObjectTypeFeed(element, elemVal){
	...
	window.objectType = elemVal;
	window.isOnline = window.isOnline;
    window.allFeedRequest = window.allFeedRequest;
    window.popularFeedFilter = window.popularFeedFilter;
    window.mostPopularFeedFilter = window.mostPopularFeedFilter;
    var loader = en4.core.loader.clone();
    loader.addClass("sd_loader");
    var filters = $("sd_feed_filters");
    var subject_guid = '<?php echo $this->subjectGuid ?>';
...
function loadAllOnlineFeed(element, elemVal){
    console.log(elemVal);
	if(!$('activity-feed')){
        return;
    }
...
<div class="sd_feed_filters" id="sd_feed_filters">
    <?php if(count($this->allCategories)): ?>
        <ul class="sd_feed_categories">
            <li class="active sd-category-0-item" data-id="0">
        ...
        <?php foreach($this->allCategories as $key => $category): ?>
            <?php //if( $key < 7 ): ?>
                <li class="<?php if(!empty($category['sub_categories'])){ echo 'has_subcategories';  } ?> sd-category-<?php echo $category['id']; ?>-item" data-id="<?php echo $category['id']; ?>">

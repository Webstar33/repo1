 ...
 */
class Sitetagcheckin_IndexController extends Core_Controller_Action_Standard {
  ...
  public function saveLocationAction() {
    ...
    if(empty($type)) {
			$type=  'just_use';
	     $checkin_params['id'] = 'just_use_li';
       $checkin_params['type'] = 'just_use';
       $checkin_params['label'] = $location;
       $checkin_params['prefixadd'] = $this->view->translate('at');
       $checkin_params['latitude'] = 0;
       $checkin_params['longitude'] = 0;
       $checkin_params['li_html'] = $this->view->translate("Just use %s", $location);
       $checkin_params['google_id'] = 1;
		}
    ...
    if (!empty($resource_guid)) {
      ...
      $location = $item->location;
		...
		//START ALBUM LOCATION FOR LOCATION PAGE.
		if($resource_type == 'album') {
			$id = 'album_id';
		} elseif($resource_type == 'album_photo') {
			$id = 'photo_id';
		}
		
		$album = Engine_Api::_()->getItem($resource_type, $resource_id);
//		if (!empty($location)) {
//			$seao_locationid = Engine_Api::_()->getDbtable('locationitems', 'seaocore')->getLocationItemId($location, '', $resource_type, $resource_id);
//
//			Engine_Api::_()->getItemTable($resource_type)->update(array('seao_locationid'=>  $seao_locationid, 'location' => $location), array("$id =?" => $resource_id));
//		} else {
//		  Engine_Api::_()->getDbtable('locationitems', 'seaocore')->delete(array('resource_id =?' => $resource_id, 'resource_type = ?' => $resource_type));
//			Engine_Api::_()->getItemTable($resource_type)->update(array('seao_locationid'=>  0, 'location' => ''), array("$id =?" => $resource_id));
//		}
    //END ALBUM LOCATION FOR LOCATION PAGE.
    ...
    //if (!empty($location)) {
      ...
      if ($resource_type == 'album') {
        ...
        $select = $actionTable->select()->from($actionTable->info('name'))
                ...
                ->where("$actionTableName.object_type =?", 'sitestore_store')
                ->order("$actionTableName.action_id DESC")
        ;
      }else {
    ...
    //FOR ALBUM TAGGING
    if ($resource_type == 'sitepage_album' || $resource_type == 'album' || $resource_type == 'sitebusiness_album' || $resource_type == 'sitegroup_album' || $resource_type == 'sitestore_album') {
  ...
  public function getLocationPhotosAction() {
    ...
    if($getEnableModuleEvent) {
			...
			$selectEvent = $eventTable->select()
								...
								->from($eventTableName, array('event_id', 'photo_id', 'parent_type', 'location','starttime', new Zend_Db_Expr ('"occurrence_id" AS occurrence_id')))
    ...
    if($getEnableModulePageEvent) {
			...
			$selectPageEvent = $pageeventTable->select()
								->setIntegrityCheck(false)
								->from($pageeventTableName, array('event_id', 'photo_id', 'parent_type', 'location','starttime', new Zend_Db_Expr ('"occurrence_id" AS occurrence_id')))
    ...
    if($getEnableModuleBusinessEvent) {
			...
			$selectBusinessEvent = $businesseventTable->select()
								->setIntegrityCheck(false)
								->from($businesseventTableName, array('event_id', 'photo_id', 'parent_type', 'location','starttime', new Zend_Db_Expr ('"occurrence_id" AS occurrence_id')))
    ...
    if($getEnableModuleGroupEvent) {
			...
			$selectGroupEvent = $groupeventTable->select()
								->setIntegrityCheck(false)
								->from($groupeventTableName, array('event_id', 'photo_id', 'parent_type', 'location','starttime', new Zend_Db_Expr ('"occurrence_id" AS occurrence_id')))
    ...
    $getEnableModuleSiteEvent = $moduleCore->isModuleEnabled('siteevent');
    $selectSiteEvent="";
    if($getEnableModuleSiteEvent) {
			...
			$selectSiteEvent = $siteeventTable->select()
								->setIntegrityCheck(false)
								->from($siteeventTableName, array('event_id', 'photo_id', 'parent_type', 'location'))
								->join($siteeventOccurrencesTableName, "$siteeventOccurrencesTableName.event_id = $siteeventTableName.event_id", array('starttime', 'occurrence_id'))
								->join($siteeventMembershipTableName, "$siteeventMembershipTableName.resource_id = $siteeventTableName.event_id", array('user_id as event_user_id'))
								->join($siteeventlocationsTableName, "$siteeventlocationsTableName.event_id = $siteeventTableName.event_id", null)
								->where($siteeventMembershipTableName . '.rsvp =?', 2)
								->where($siteeventMembershipTableName . '.user_id =?', $subject->user_id)
								->where("$siteeventlocationsTableName.location_id = ?", $location_id)
								->where("$siteeventOccurrencesTableName.starttime <= ?", date('Y-m-d H:i:s'));
      $event['siteevent']  =  $selectSiteEvent;
    }    
    ...
    if(isset($event['siteevent'])) {
      $union_event_array[] = $selectSiteEvent;
    }
  ...
  public function viewMapAction() {
    ...
    $this->view->userGeoSettings = Engine_Api::_()->seaocore()->geoUserSettings('sitetagcheckin');
    if($item->type == 'sitetagcheckin_album_photo_new') {
      ...
      $photoparams = (array) $addlocationsTable->fetchRow($select)->params;
       ...
       if(is_array($photoparams) && isset($photoparams['checkin']) && !empty($photoparams['checkin']) && !empty($photoparams['checkin']['label'])) {
        $params = $photoparams;
      }

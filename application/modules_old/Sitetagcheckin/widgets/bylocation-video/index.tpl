...
<script type="text/javascript">
  ...
  var paginatePageLocations = function(page) {
		var  formElements = document.getElementById('video_filter_form');
		var parms = formElements.toQueryString(); 
		var param = (parms ? parms + '&' : '') + 'is_ajax=1&format=html&page=' + page;
		document.getElementById('video_location_loding_image').style.display ='';
    var url = en4.core.baseUrl + 'widget/index/mod/sitetagcheckin/name/bylocation-video';
    clearOverlays();
    gmarkers = [];
    en4.core.request.send(new Request.HTML({
      ...
      onSuccess :function(responseTree, responseElements, responseHTML, responseJavaScript) {
				document.getElementById('video_location_loding_image').style.display ='none';
				document.getElementById('videolocation_location_map_anchor').getParent().innerHTML = responseHTML;
				setMarker();
      }
    })
    );
...
<div class="eventlocation_browse_location" id="eventlocation_browse_location" >
	<?php if (count($this->paginator) > 0): ?>
		...
		<div id="eventlocation_map_container" class="eventlocation_map_container absolute">
...
<?php endif; ?>
  ...
  <a id="videolocation_location_map_anchor" class="pabsolute"></a>
		<?php if (count($this->paginator) > 0): ?>
			<ul class="seaocore_browse_list" id="seaocore_browse_list"><?php if (!empty($this->is_ajax)) : ?>	
              ...
              <?php
              if ($item->photo_id) {
                echo $this->htmlLink($item->getHref(), $this->itemPhoto($item, 'thumb.normal'), array( 'class' => !empty($item->location)? "seao_common_add_tooltip_link marker_photo_".$item->album_id :'un_location_eventlocation'));
              } else {
                echo '<img alt="" src="' . $this->layout()->staticBaseUrl . 'application/modules/Video/externals/images/video.png">';
              }
              ?>
							<?php //echo $this->htmlLink($item->getHref(), $this->itemPhoto($item, 'thumb.normal'), array('title' => $item->getTitle(), 'target' => '_parent', 'class' => !empty($item->location)? "seao_common_add_tooltip_link marker_photo_".$item->video_id :'un_location_eventlocation seao_common_add_tooltip_link', 'rel'=>'video'.' '.$item->video_id)); ?>
						</div>
						<div class="video_stats fright">
						...
						<div class='seaocore_browse_list_info'>
							<div class='seaocore_browse_list_info_title'>
								<?php echo $this->htmlLink($item->getHref(), $item->getTitle(), array('title' => $item->getTitle(), 'target' => '_parent', 'class' =>!empty($item->location)? "seao_common_add_tooltip_link marker_".$item->video_id :'un_location_eventlocation')); ?>
							</div>
              ...
              <?php if(!empty($item->location)): ?>
								...
								<?php if (!empty($item->distance) && isset($item->distance)): ?>
									<div class="seaocore_browse_list_info_stat">
										<?php if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('sitetagcheckin.proximity.search.kilometer')): ?>
			...
			<div class="clr eventlocation_browse_location_paging" style="margin-top:10px;">
				...
				<?php if( count($this->paginator) > 1 ): ?>
					<div class="fleft" id="video_location_loding_image" style="display: none;margin:5px;">
		...
		<?php else: ?>
			<div class="tip"> 
				<span><?php echo $this->translate('Nobody has created a video with that criteria.'); ?></span>
			</div>
		<?php endif; ?>
		<?php if (empty($this->is_ajax)) : ?>
	</div>
...
<script type="text/javascript" >
  ...
  window.addEvent('domready',function() {
    //smallLargeMap(1);
  //  var Clientwidth = $('global_content').getElement(".layout_sitetagcheckin_bylocation_video").clientWidth;
...
<script type="text/javascript" >
  ...
  function createMarker(latlng, name, html, video_title, video_id) {
    ...
    //Show tooltip on the mouse over.
	  $$('.marker_' + video_id).each(function(locationMarker) {
    ...
    //Show tooltip on the mouse over.
	  $$('.marker_photo_' + video_id).each(function(locationMarker) {
			locationMarker.addEvent('mouseover',function(event) {
				google.maps.event.trigger(map, 'resize');
				map.setCenter(marker.position);
				infoBubbles.open(map,marker);
...
<?php if (!empty($location->location)) : ?>
	...
	var video_id = '<?php echo  $location->video_id;  ?>';
	var sponsored = 0;
	// create the marker

	<?php $video_id = $location->video_id; ?>
	var contentString = '<div id="content">'+
		...
		'<?php echo $this->htmlLink($location->getHref(), $this->itemPhoto($location, 'thumb.icon'), array('title' => $location->getTitle(), 'target' => '_parent')); ?>'+
		'</div>'+
		...
		' </div>'+
		'</div>';
	  var marker = createMarker(point,sponsored,contentString,"<?php echo str_replace('"',' ',$location->getTitle()); ?>", video_id);
<?php endif; ?>
<?php   endforeach; ?>

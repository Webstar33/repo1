...
class User_Plugin_Signup_Fields extends Core_Plugin_FormSequence_Abstract
{
  ...
  public function onSubmit(Zend_Controller_Request_Abstract $request)
  {
    ...
    if( $this->getForm()->isValid($request->getPost()) )
    {
      ...
      $this->getSession()->data['locationParams'] = $request->getParam("locationParams");
  ...
  public function onProcess()
  {
    ...
    try{
        ...
        $data = $this->getSession()->data;
        if(!empty($data['locationParams'])){
            $locationParams = (array)@json_decode($data['locationParams']);
            if(!empty($locationParams['latitude'])){
                $table = Engine_Api::_()->getDbTable("locationitems","seaocore");
                $location = $table->createRow();
                $locationParams['resource_type'] = $user->getType();
                $locationParams['resource_id'] = $user->getIdentity();

 ...
 */
class Advancedactivity_Widget_PostFeedButtonController extends Engine_Content_Widget_Abstract {
...
// 
    ...
    if ($this->view->category_id && Engine_Api::_()->hasModuleBootstrap('advancedactivitypost'))
...
// Assign the composing values
    ...
    foreach (Zend_Registry::get('Engine_Manifest') as $data) {
      ...
      if (!empty($data['composer']['advanced_facebook']) && !$this->isFacebookEnable($subject)) {
        continue;
      }
      ...
      foreach ($data['composer'] as $type => $config) {
        $key = $type . 'XXX' . $config['script'][1];
          if(!in_array($type , array('advanced_facebook','advanced_twitter','advanced_linkedin','tag')) && !in_array($key, $composerList)){
          continue;
          } 
    ...
    $this->view->composePartials = array_merge(array_flip($composerList), $composePartials);
// Get lists

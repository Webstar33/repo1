    ...
    public function postAction()
    {
        ...
        try {
            ...
            if (!$attachmentCount && $viewer->isSelf($subject)) {
                ...
                $action = $actionTable->addActivity($viewer, $subject, $activityType, $body, array('count' => $attachmentCount));
            ...
            /****************SD Dev1 custom code for feed categories***************/
            
            if($action && !empty($postData['category_id'])){
                $action->category_id = $postData['category_id'];
                if(!empty($postData['subcategory_id'])){
                    $action->subcategory_id = $postData['subcategory_id'];
                }
                $action->save();
            }

    ...
    public function indexAction()
    {
        ...
        */
        ...
        foreach ($this->_packageManager->listInstalledPackages(array('caching' => false)) as $installedPackage) {
            ...
            $sliceName = array_slice($installedPackage->getAuthors(), 0, 1);
            $companyName = array_shift($sliceName);

            if (!isset($installedPackages[$companyName])) {
                $installedPackages[$companyName] = array();
            }

            if (!isset($upgradeablePackages[$companyName])) {
                $upgradeablePackages[$companyName] = array();
            }

            if (!isset($pendingPackages[$companyName])) {
                $pendingPackages[$companyName] = array();
            }

            if ($upgradeable) {
                $upgradeablePackages[$companyName][] = $installedPackageInfo;
            } elseif ($pending) {
                $pendingPackages[$companyName][] = $installedPackageInfo;
            } else {
                $installedPackages[$companyName][] = $installedPackageInfo;
            }
        }
